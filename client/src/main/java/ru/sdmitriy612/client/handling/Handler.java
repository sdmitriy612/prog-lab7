package ru.sdmitriy612.client.handling;

import ru.sdmitriy612.client.Session;
import ru.sdmitriy612.client.commands.ClientCommandsMap;
import ru.sdmitriy612.client.commands.Command;
...
public class Handler {
    ...
    public String handle(String requestString){
        ...
        UserAuthorization sessionAuthorization = Session.getInstance().getUserAuthorization();
        ...
        switch (response.responseType()) {
            ...
            case ERROR -> {
                HashMap<String, Command> clientCommandsMap = ClientCommandsMap.getCommands();
                if(clientCommandsMap.containsKey(commandName)){
                    response = clientCommandsMap.get(request.commandName()).execute(request);
                }
            }
            case AUTHORIZE -> {
